...
namespace SignOn {
    ...
    void CryptoManager::addKeyToKeychain(const QByteArray &key)
    {
    ...
    void CryptoManager::removeKeyFromKeychain(const QByteArray &key)
    {
        QSettings keychain(keychainFilePath(), QSettings::IniFormat);
    ...
    bool CryptoManager::keychainContainsKey(const QByteArray &key)
    {
        QSettings keychain(keychainFilePath(), QSettings::IniFormat);
        foreach (QString keyIt, keychain.allKeys()) {
            if (keychain.value(keyIt).toByteArray() == key)
